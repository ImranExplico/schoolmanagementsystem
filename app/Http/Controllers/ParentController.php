...
class ParentController extends Controller
{
    ...
    public function teacherList()
    {
        $teachers = User::where('role_id', 3)
            ...
            ->paginate(10);
        return view('parent.user.teacher_list', compact('teachers'));
    }
    ...
    public function childList()
    {
        $students = User::where('role_id', 7)
            ...
            ->where('school_id', auth()->user()->school_id)
            ->paginate(10);
        return view('parent.user.child_list', compact('students'));
    }
    ...
    { //parent
        ...
        if (count($request->all()) > 0) {
            ...
            if ($selected_status != "all") {
                $invoices = StudentFeeManager::where('timestamp', '>=', $date_from)->where('timestamp', '<=', $date_to)->where('status', $selected_status)->where('parent_id', auth()->user()->id)->where('session_id', $active_session->id)->get();
            } else if ($selected_status == "all") {
                $invoices = StudentFeeManager::where('timestamp', '>=', $date_from)->where('timestamp', '<=', $date_to)->where('school_id', auth()->user()->school_id)->where('parent_id', auth()->user()->id)->where('session_id', $active_session->id)->get();
            }
            ...
            $invoices = StudentFeeManager::where('timestamp', '>=', $date_from)->where('timestamp', '<=', $date_to)->where('parent_id', auth()->user()->id)->where('school_id', auth()->user()->school_id)->where('session_id', $active_session->id)->get();
    ...
    public function list_of_attendence(Request $request)
    {
        ...
        return view('parent.attendence.list_of_attendence', ['student_data' => $student_data]);
    }

    public function parentdailyAttendanceFilter(Request $request)
    {
        ...
        $attendance_of_students = DailyAttendances::whereBetween('timestamp', [$first_date, $last_date])->where(['student_id' => $data['student_id'], 'school_id' => auth()->user()->school_id, 'session_id' => $active_session->id])->get()->toArray();
        $userName = User::find($data['student_id']);


        return view('parent.attendence.load_table', ['userName' => $userName, 'page_data' => $page_data, 'attendance_of_students' => $attendance_of_students]);
    }
