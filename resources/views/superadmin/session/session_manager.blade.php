        ...
        <div class="col-12">
            ...
            >
                ...
                <div class="export-btn-area">
                    <a href="javascript:;" class="export_btn" onclick="rightModal('{{ route('superadmin.create.session') }}', '{{ get_phrase('Create Session') }}')"><i class="bi bi-plus"></i>{{ get_phrase('Add Session') }}</a>
                </div>
            </div>
        </div>
    </div>
						...
						<table id="basic-table" class="table eTable">
			                <thead>
			                    <tr>
			                        ...
			                        <th>{{ get_phrase('Starting Date') }}</th>
			                        <th>{{ get_phrase('Ending Date') }}</th>
			                ...
			                <tbody class="table_body">
			                    ...
			                    <tr>
			                        ...
			                        <td><strong>{{ date('m/d/Y', strtotime($session['starting_date'])) }}</strong></td>
			                        <td><strong>{{ date('m/d/Y', strtotime($session['ending_date'])) }}</strong></td>
			                        {{-- <td>
			                        ...
			                        <td class="text-start">
			                        	<div class="adminTable-action">
					                        ...
					                        >
					                          <li>
					                            <a class="dropdown-item" href="javascript:;" onclick="rightModal('{{ route('superadmin.edit.session', ['id' => $session->id]) }}', '{{ get_phrase('Edit Session') }}')">{{ get_phrase('Edit') }}</a>
					                          </li>
					                          <li>
					                            <a class="dropdown-item" href="javascript:;" onclick="confirmModal('{{ route('superadmin.session.delete', ['id' => $session->id]) }}', 'undefined');">{{ get_phrase('Delete') }}</a>
					                          </li>
					                        </ul>
					                    </div>
			                        </td>
...
<script type="text/javascript">
  ...
  "use strict";
	...
	function makeSessionActive() {
	    ...
	    let url = "{{ route('superadmin.session_manager.active_session', ['id' => ":session_id"]) }}";
