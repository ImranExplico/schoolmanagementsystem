	...
	<form method="POST" enctype="multipart/form-data" class="d-block ajaxForm" action="{{ route('admin.noticeboard.update', ['id' => $notice->id]) }}">
	    ...
	    <div class="form-row">
			...
			<div class="fpb-7 pt-2">
				<button class="btn btn-block btn-primary float-right" type="submit">{{ get_phrase('Update') }}</button>

				<a href="javascript:void(0)" class="btn btn-block btn-danger float-end" onclick="showNoticeDeleteModal()">{{ get_phrase('Delete') }}</a>
			</div>
		</div>
	</form>
</div>
