		...
		<tbody>
		    ...
		    ?>
		      <tr>
		        ...
		        <td>
		          	<input type="checkbox" value="{{ $permission['marks'] }}" id="{{ $teacher['id'].'1' }}" data-switch="success" onchange="togglePermission(this.id, 'marks', '{{ $teacher['id'] }}')" {{ $permission['marks'] == 1 ? 'checked':'' }}>
                	<label for="{{ $teacher['id'].'1' }}" data-on-label="Yes" data-off-label="No">
		        </td>
		        <td>
		          	<input type="checkbox" value="{{ $permission['attendance'] }}" id="{{ $teacher['id'].'3' }}" data-switch="success" onchange="togglePermission(this.id, 'attendance', '{{ $teacher['id'] }}')" {{ $permission['attendance'] == 1 ? 'checked':'' }}>
                	<label for="{{ $teacher['id'].'3' }}" data-on-label="Yes" data-off-label="No">
		        </td>
		      </tr>
		    @endforeach
		</tbody>
