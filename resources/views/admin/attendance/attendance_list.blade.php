...
$attendance_of_students = DailyAttendances::where(['class_id' => $page_data['class_id'], 'section_id' => $page_data['section_id'], 'school_id' => auth()->user()->school_id])->get();

$no_of_users = DailyAttendances::where(['class_id' => $page_data['class_id'], 'section_id' => $page_data['section_id'], 'school_id' => auth()->user()->school_id])->distinct()->count('student_id');
        ...
        <form method="GET" enctype="multipart/form-data" class="d-block ajaxForm" action="{{ route('admin.daily_attendance.filter') }}">
          <div class="att-filter d-flex flex-wrap">
            ...
            <div class="att-filter-btn">
              <button class="btn btn-block btn-secondary" type="submit" >{{ get_phrase('Filter') }}</button>
            </div>
